defmodule Celixir.CelSpecConformanceTest do
  @moduledoc """
  Runs the official cel-spec conformance tests from textproto files.

  Each textproto file generates a describe block, each section a nested describe,
  and each test case an ExUnit test.
  """
  use ExUnit.Case
  ...
  # Dynamically generate tests from textproto files
  for file <- Path.wildcard(Path.join(@testdata_dir, "*.textproto")) do
    ...
    describe "cel-spec: #{file_name}" do
  ...
  defp run_cel_spec_test(_file, _section, test) do
  ...
  defp convert_value({:int64, v}), do: v
  defp convert_value({:uint64, v}), do: v
  defp convert_value({:double, v}) when is_number(v), do: v * 1.0
  defp convert_value({:double, v}) when is_atom(v), do: v
  defp convert_value({:bool, v}), do: v
  defp convert_value({:string, v}), do: v
  defp convert_value({:bytes, v}), do: {:cel_bytes, v}
  defp convert_value({:null, _}), do: nil

  defp convert_value({:duration, seconds, nanos}) do
    micros = seconds * 1_000_000 + div(nanos, 1000)
    Celixir.Types.Duration.new(micros)
  end

  defp convert_value({:timestamp, seconds, nanos}) do
    micros = seconds * 1_000_000 + div(nanos, 1000)
    {:ok, dt} = DateTime.from_unix(micros, :microsecond)
    Celixir.Types.Timestamp.new(dt)
  end

  defp convert_value({:list, items}), do: Enum.map(items, &convert_value/1)

  defp convert_value({:map, entries}) do
    Map.new(entries, fn {k, v} -> {convert_value(k), convert_value(v)} end)
  end

  # Proto-typed bindings (from object_value in textproto)
  defp convert_value({:object, type_name, fields}) do
    convert_proto_object(type_name, fields)
  end

  defp convert_value(nil), do: nil

  # Convert proto object bindings to CEL values
  defp convert_proto_object(type_name, fields) do
    # Wrapper types → unwrap to native value
    case type_name do
  ...
  defp convert_proto_value(fields) do
    cond do
      Map.has_key?(fields, "null_value") ->
        nil

      Map.has_key?(fields, "bool_value") ->
        Map.get(fields, "bool_value")

      Map.has_key?(fields, "number_value") ->
        Map.get(fields, "number_value") * 1.0

      Map.has_key?(fields, "string_value") ->
        Map.get(fields, "string_value")

      Map.has_key?(fields, "list_value") ->
        convert_proto_list(Map.get(fields, "list_value"))

      Map.has_key?(fields, "struct_value") ->
        convert_proto_struct(Map.get(fields, "struct_value"))
  ...
  defp convert_proto_struct(fields) do
    case Map.get(fields, "fields") do
      m when is_map(m) ->
        Map.new(m, fn {k, v} -> {k, convert_proto_value(v)} end)
  ...
  defp assert_value_match(nil, _actual, _expr), do: :ok

  defp assert_value_match({:null, _}, actual, expr) do
    assert(actual == nil, "#{expr}: expected null, got #{inspect(actual)}")
  end

  defp assert_value_match({:bool, expected}, actual, expr) do
    assert(actual == expected, "#{expr}: expected #{expected}, got #{inspect(actual)}")
  end

  defp assert_value_match({:int64, expected}, actual, expr) do
    actual_int =
      case actual do
        {:cel_int, v} -> v
  ...
  defp assert_value_match({:uint64, expected}, actual, expr) do
    actual_uint =
      case actual do
        {:cel_uint, v} -> v
        v when is_integer(v) -> v
  ...
  defp assert_value_match({:double, expected}, actual, expr) when is_float(actual) do
    assert(
  ...
  defp assert_value_match({:double, expected}, actual, expr) when is_atom(actual) do
    assert(actual == expected, "#{expr}: expected #{expected}, got #{inspect(actual)}")
  end

  defp assert_value_match({:string, expected}, actual, expr) do
    assert(actual == expected, "#{expr}: expected #{inspect(expected)}, got #{inspect(actual)}")
  end

  defp assert_value_match({:bytes, expected}, actual, expr) do
    actual_bytes =
      case actual do
        {:cel_bytes, b} -> b
        b -> b
  ...
  defp assert_value_match({:list, expected_items}, actual, expr) when is_list(actual) do
    assert(
  ...
  defp assert_value_match({:map, expected_entries}, actual, expr) when is_map(actual) do
  ...
  defp assert_value_match({:type, expected_type}, actual, expr) do
    actual_type =
      case actual do
        {:cel_type, name} -> name
  ...
  # Proto object value match — compare the wrapped value
  defp assert_value_match({:object, type_name, expected_fields}, actual, expr) do
    case actual do
      {:cel_struct, actual_type, actual_fields} ->
        # For structs, check type matches and all expected fields are present
        assert(
          actual_type == type_name or
            String.ends_with?(actual_type, "." <> type_name),
          "#{expr}: expected struct type #{type_name}, got #{actual_type}"
        )

        Enum.each(expected_fields, fn {k, v} ->
          assert(
            Map.has_key?(actual_fields, k),
            "#{expr}: expected field #{k} not found in struct"
          )
      ...
      _ ->
        ...
        assert(
          actual == converted,
          "#{expr}: expected #{inspect(converted)}, got #{inspect(actual)}"
        )
    end
  end

  defp assert_value_match(expected, actual, expr) do
    flunk("#{expr}: unhandled value match — expected #{inspect(expected)}, got #{inspect(actual)}")
  end

  # Compare proto field values, handling wrapper/value types
  defp assert_proto_field_match(field_name, expected, actual, expr) do
    if proto_value_matches?(expected, actual) do
      ...
      assert(
        false,
        "#{expr}: field #{field_name}: expected #{inspect(expected)}, got #{inspect(actual)}"
      )
    end
  end

  # Check if a proto expected value matches an actual CEL value
  defp proto_value_matches?(expected, actual) when expected == actual, do: true

  # Wrapper type: %{"value" => inner} matches unwrapped value
  defp proto_value_matches?(%{"value" => inner}, actual),
    do: normalize_for_compare(actual) == normalize_for_compare(inner)

  # Empty map matches anything (default/zero value)
  defp proto_value_matches?(%{} = e, _actual) when map_size(e) == 0, do: true

  # Proto Value types: %{"bool_value" => v} matches v directly
  ...
  # ListValue structure: %{"values" => [value_maps...]} matches a plain list
  defp proto_value_matches?(%{"values" => expected_vals}, actual) when is_list(actual) do
    length(expected_vals) == length(actual) and
      expected_vals
      |> Enum.zip(actual)
      |> Enum.all?(fn {e, a} -> proto_value_matches?(e, a) end)
  end

  # Struct structure: %{"fields" => %{key => value_map}} matches a plain map
